...
// ======================
// DEV VERIFY (for testing in development)
// ======================
const devVerify = async (req, res) => {
  try {
    res.json({ 
      success: true, 
      message: 'Development verification bypass successful.' 
    });
  } catch (error) {
...
const getProfile = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res.status(400).json({ success: false, error: 'Invalid user token' });
    }
